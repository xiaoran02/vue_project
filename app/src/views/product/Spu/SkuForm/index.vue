...
export default {
  ...
  data() {
    return {
      ...
      spu: {},
      //收集图片数据的字段:但是需要注意，
      //收集的数据目前缺少isDefault字段，将来提交给服务器数据的时候，需要整理参数
      imageList: [],
    };
  },
  methods: {
    ...
    handleSelectionChange(params) {
      //获取到用户选中图片的信息数据，但是需要注意，当前收集的数据当中，缺少Default字段
    ...
    save() {
      ...
      //整理平台属性的数据
      skuInfo.skuAttrValueList = this.attrInfoList.length > 0 ? attrInfoList.reduce((prev, item) => {
      ...
      //整理销售属性的数据
      skuInfo.skuSaleAttrValueList = this.spuSaleAttrList.length > 0 ? spuSaleAttrList.reduce((prev, item) => {
        if (item.attrIdAndValueId) {
